...
ConstellationTuple = collections.namedtuple(
    'ConstellationTuple', 'presence corners pattern_presence '
    'pattern_id')
...
def create_numpy(
    ...
    pattern = [
        np.array([[1 + 2 * sm, 1 + 2 * sm, 1], [1, 1, 1], [1 + 2 * sm, 1, 1],
                  [1, 1 + 2 * sm, 1]]),  # square
        np.array([[1, 1 + sm, 1], [1 + 2 * sm, 1, 1],
                  [1 + 2 * sm, 1 + 2 * sm, 1]]),  # triangle
        np.array([[1, 1, 1], [1 + sm, 1 - 2 * sm, 1], [1 + 2 * sm, 1 - sm, 1],
                  [1 + 2 * sm, 1 + sm, 1], [1 + sm, 1 + 2 * sm,
                                            1]]),  # pentagon
    ...
    for i in range(len(which_patterns)):
        ...
        degree = (np.random.random(
            (size_n)) - .5) * 2. * np.pi * rotation_percent
        ...
        random_corer_choice = np.tile(random_pattern_choice,
                                      (1, corners.shape[1]))
    ...
    minibatch = dict(corners=capsules,
...
def create(
    ...
    def generator(n_batches=100):
        ...
        while True:
            ...
            batch = create_numpy(n, shuffle_corners, gaussian_noise,
                                 max_translation, rotation_percent, scale,
