...
flags.DEFINE_boolean(
    'use_alpha_channel', False, 'Learns per-pixel mixing '
    'proportions for every template; otherwise mixing '
    'probabilities are constrained to have the same value as '
    'image pixels.')

flags.DEFINE_string('template_nonlin', 'relu1',
                    'Nonlinearity used to normalize'
                    ' part templates.')
flags.DEFINE_string(
    'color_nonlin', 'relu1', 'Nonlinearity used to normalize'
...
def make_scae(config):
    ...
    obj_decoder = ImageCapsule(config.n_obj_caps,
                               2,
                               config.n_part_caps,
    ...
    model = ImageAutoencoder(
        ...
        prior_within_example_sparsity_weight=config.
        prior_within_example_sparsity_weight,  # pylint:disable=line-too-long
        prior_between_example_sparsity_weight=config.
        prior_between_example_sparsity_weight,  # pylint:disable=line-too-long
        posterior_sparsity_loss_type='entropy',
        posterior_within_example_sparsity_weight=config.
