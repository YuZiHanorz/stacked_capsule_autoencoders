...
class CapsuleImageEncoder(snt.AbstractModule):
    ...
    OutputTuple = collections.namedtuple(  # pylint:disable=invalid-name
        'PrimaryCapsuleTuple', 'pose feature presence presence_logit '
        'img_embedding')
    ...
    def __init__(self,
        ...
        self._encoder_kwargs = dict(n_layers=2,
                                    n_heads=4,
                                    n_dims=32,
                                    layer_norm=False)
...
class TemplateBasedImageDecoder(snt.AbstractModule):
    ...
    def make_templates(self, n_templates=None, template_feature=None):
        ...
        if self._templates is not None:
            if n_templates is not None and self._templates.shape[
                    1] != n_templates:
                raise ValueError
            ...
            with self._enter_variable_scope():
                ...
                template_shape = ([1, n_templates] +
                                  list(self._template_size) + [n_dims])
                ...
                q = np.linalg.qr(q)[0]
                q = q[:n_templates, :n_elems].reshape(template_shape).astype(
                    np.float32)
        ...
        if template_feature is not None:
            ...
            if self._colorize_templates:
                ...
                template_color = mlp(template_feature)[:, :, tf.newaxis,
                                                       tf.newaxis]
